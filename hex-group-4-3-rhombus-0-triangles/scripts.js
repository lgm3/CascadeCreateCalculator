...
window.onload = () => {
  ...
  calculateButtonEl.addEventListener("click", () => {
    ...
    const linearInchOfTriangle = linearInchOfHexLog * 0;
    const linearInchOfRhombus = linearInchOfHexLog * 3;
    const nTwelveInchTriangleSections = Math.ceil(
      linearInchOfTriangle / panelLength
    );
    const nTwelveInchRhombusSections = Math.ceil(
      linearInchOfRhombus / panelLength
    );
    const panelWidth =
      nTwelveInchTriangleSections * triangleSide +
      (nTwelveInchTriangleSections - 1) * tableSawKerf +
      (2 + nTwelveInchRhombusSections) * triangleSide +
      (nTwelveInchRhombusSections - 1) * tableSawKerf;
